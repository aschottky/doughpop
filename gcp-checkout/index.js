...
export async function createCheckoutSession(req, res) {
  ...
  // ── Resolve refund: find order from Stripe refund/charge ─────────────────
  if (body.action === 'resolve-refund') {
    if (!stripeSecretKey) { sendError('Stripe not configured', 500); return }
    try {
      const { userId } = await verifyJWT(req, body, supabaseUrl)
      const supabase = createClient(supabaseUrl, supabaseServiceKey)
      ...
      if (!refund_id) { sendError('Missing refund_id', 400); return }
      ...
      if (chargeId) {
        const { data: payment } = await supabase.from('payments')
          ...
          .or(`stripe_charge_id.eq.${chargeId},reference_id.eq.${chargeId}`)
          .maybeSingle()
        if (payment) {
          ...
          const { data: order } = await supabase.from('orders')
            ...
            .maybeSingle()
          if (order) { orderId = order.id; orderNumber = order.order_number }
        }
      }
      ...
      const { data: refundRecord, error: refundError } = await supabase.from('refunds').insert({
        baker_id: userId,
        payment_id: paymentId,
        order_id: orderId,
        invoice_id: invoiceId,
        amount: refund.amount / 100,
        reason: refund.reason || body.reason || null,
        processed_by: userId,
      ...
      if (refundError) { sendError(refundError.message, 500); return }

      setCors(res); res.status(200).json({
        success: true,
        refund: refundRecord,
        resolved: { orderId, orderNumber, invoiceId, paymentId, chargeId },
      }); return
    } catch (err) {
      sendError(err?.message || 'Failed to resolve refund', err?.status || 500); return
    }
  }
